...
function determine_images_in_out_x86_64() {
    ...
    local base_nvidia_image="${smartsim_REPO}:cuda${CUDA_LITE}-cudnn${cudnn_ver}-trt${trt_ver}-devel-${UBUNTU_LTS}-${arch}"
    ...
    if [[ "${stage}" == "base" ]]; then
        ...
        if [[ "${dist}" == "stable" ]]; then
            IMAGE_OUT="${smartsim_REPO}:cyber-${arch}-${UBUNTU_LTS}-${timestamp}"
        else
            IMAGE_OUT="${smartsim_REPO}:cyber-${arch}-${UBUNTU_LTS}-testing-${timestamp}"
        fi
    elif [[ "${stage}" == "dev" ]]; then
        if [[ "${dist}" == "stable" ]]; then
            IMAGE_IN="${smartsim_REPO}:cyber-${arch}-${UBUNTU_LTS}-${PREV_IMAGE_TIMESTAMP}"
            IMAGE_OUT="${smartsim_REPO}:dev-${arch}-${UBUNTU_LTS}-${timestamp}"
        else
            IMAGE_IN="${smartsim_REPO}:cyber-${arch}-${UBUNTU_LTS}-testing-${PREV_IMAGE_TIMESTAMP}"
            IMAGE_OUT="${smartsim_REPO}:dev-${arch}-${UBUNTU_LTS}-testing-${timestamp}"
        fi
    elif [[ "${stage}" == "runtime" ]]; then
        if [[ "${dist}" == "stable" ]]; then
            if [[ "${TARGET_GPU}" == "amd" ]]; then
            ...
            DEV_IMAGE_IN="${smartsim_REPO}:dev-${arch}-${UBUNTU_LTS}-${PREV_IMAGE_TIMESTAMP}"
            IMAGE_OUT="${smartsim_REPO}:runtime-${arch}-${UBUNTU_LTS}-${timestamp}"
        else
...
function determine_images_in_out_aarch64() {
    ...
    if [[ "${stage}" == "base" ]]; then
        ...
        IMAGE_OUT="${smartsim_REPO}:${BASE_FMT}-${timestamp}"
    elif [[ "${stage}" == "cyber" ]]; then
        IMAGE_IN="${smartsim_REPO}:${BASE_FMT}-${PREV_IMAGE_TIMESTAMP}"
        IMAGE_OUT="${smartsim_REPO}:${CYBER_FMT}-${timestamp}"
    elif [[ "${stage}" == "dev" ]]; then
        IMAGE_IN="${smartsim_REPO}:${CYBER_FMT}-${PREV_IMAGE_TIMESTAMP}"
        IMAGE_OUT="${smartsim_REPO}:dev-aarch64-nvidia-${UBUNTU_LTS}-${timestamp}"
    elif [[ "${stage}" == "runtime" ]]; then
...
function check_arguments() {
    ...
    if [[ "${TARGET_DIST}" == "stable" ]]; then
        ...
        echo "Unknown smartsim_DIST: ${TARGET_DIST}. Exit..."
        exit 1
    fi
...
function docker_build_preview() {
    ...
    echo "|  INSTALL_MODE=${INSTALL_MODE}, GEOLOC=${TARGET_GEOLOC}, smartsim_DIST=${TARGET_DIST}"
    echo "=====.=====.=====.=====.=====.=====.=====.=====.=====.=====.=====.=====.====="
}
...
function docker_build_run() {
    ...
    if [[ "${TARGET_STAGE}" == "base" ]]; then
        ...
        build_args="${build_args} --build-arg smartsim_DIST=${TARGET_DIST}"
