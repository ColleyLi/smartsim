...
export RCFILES_DIR="/opt/smartsim/rcfiles"
export smartsim_DIST="${smartsim_DIST:-stable}"

export PKGS_DIR="/opt/smartsim/pkgs"
export SYSROOT_DIR="/opt/smartsim/sysroot"

export smartsim_PROFILE="/etc/profile.d/smartsim.sh"
export smartsim_LD_FILE="/etc/ld.so.conf.d/smartsim.conf"
export DOWNLOAD_LOG="/opt/smartsim/build.log"
...
function smartsim_environ_setup() {
    ...
    if [ ! -f "${smartsim_LD_FILE}" ]; then
        echo "${SYSROOT_DIR}/lib" | tee -a "${smartsim_LD_FILE}"
    fi
    if [ ! -f "${smartsim_PROFILE}" ]; then
        cp -f /opt/smartsim/rcfiles/smartsim.sh.sample "${smartsim_PROFILE}"
        echo "add_to_path ${SYSROOT_DIR}/bin" >> "${smartsim_PROFILE}"
    fi
