...
CACHE_ROOT_DIR="${smartsim_ROOT_DIR}/.cache"

DOCKER_REPO="smartsimauto/smartsim"
...
ROCM_DOCKER_REPO="rocmsmartsim/smartsim"
    ...
    while [ $# -gt 0 ]; do
        ...
        case "${opt}" in
            ...
            stop)
                ...
                stop_all_smartsim_containers "-f"
                exit 0
                ;;
            *)
...
function setup_devices_and_mount_local_volumes() {
    ...
    source "${smartsim_ROOT_DIR}/scripts/smartsim_base.sh"
    setup_device

    local volumes="-v $smartsim_ROOT_DIR:/smartsim"

    [ -d "${smartsim_CONFIG_HOME}" ] || mkdir -p "${smartsim_CONFIG_HOME}"
    volumes="-v ${smartsim_CONFIG_HOME}:${smartsim_CONFIG_HOME} ${volumes}"

    local teleop="${smartsim_ROOT_DIR}/../smartsim-teleop"
    if [ -d "${teleop}" ]; then
        volumes="${volumes} -v ${teleop}:/smartsim/modules/teleop ${volumes}"
    fi
    local smartsim_tools="${smartsim_ROOT_DIR}/../smartsim-tools"
    if [ -d "${smartsim_tools}" ]; then
        volumes="${volumes} -v ${smartsim_tools}:/tools"
    fi
...
function restart_map_volume_if_needed() {
    local map_name="$1"
    local map_version="$2"
    local map_volume="smartsim_map_volume-${map_name}_${USER}"
    local map_path="/smartsim/modules/map/data/${map_name}"
...
function mount_other_volumes() {
    ...
    # AUDIO
    local audio_volume="smartsim_audio_volume_${USER}"
    local audio_image="${DOCKER_REPO}:data_volume-audio_model-${TARGET_ARCH}-latest"
    local audio_path="/smartsim/modules/audio/data/"
    docker_restart_volume "${audio_volume}" "${audio_image}" "${audio_path}"
    volume_conf="${volume_conf} --volume ${audio_volume}:${audio_path}"

    #TRAFFIC_LIGHT_DETECTION
    local tl_detection_volume="smartsim_tl_detection_volume_${USER}"
    local tl_detection_image="${DOCKER_REPO}:traffic_light-detection_caffe_model-${TARGET_ARCH}-latest"
    local tl_detection_path="/smartsim/modules/perception/production/data/perception/camera/models/traffic_light_detection/tl_detection_caffe"
    ...
    local tl_horizontal_volume="smartsim_tl_horizontal_volume_${USER}"
    local tl_horizontal_image="${DOCKER_REPO}:traffic_light-horizontal_caffe_model-${TARGET_ARCH}-latest"
    local tl_horizontal_path="/smartsim/modules/perception/production/data/perception/camera/models/traffic_light_recognition/horizontal_caffe"
    ...
    #TRAFFIC_LIGHT_RECOGNITION
    local tl_quadrate_volume="smartsim_tl_quadrate_volume_${USER}"
    local tl_quadrate_image="${DOCKER_REPO}:traffic_light-quadrate_caffe_model-${TARGET_ARCH}-latest"
    local tl_quadrate_path="/smartsim/modules/perception/production/data/perception/camera/models/traffic_light_recognition/quadrate_caffe"
    ...
    #TRAFFIC_LIGHT_RECOGNITION
    local tl_recognition_volume="smartsim_tl_recognition_volume_${USER}"
    local tl_recognition_image="${DOCKER_REPO}:traffic_light-recognition_caffe_model-${TARGET_ARCH}-latest"
    local tl_recognition_path="/smartsim/modules/perception/production/data/perception/camera/models/traffic_light_recognition/vertical_caffe"
    ...
    local yolo_path="/smartsim/modules/perception/production/data/perception/camera/models/yolo_obstacle_detector/3d-r4-half_caffe"
    ...
    local cnnseg_path="/smartsim/modules/perception/production/data/perception/lidar/models/cnnseg/cnnseg128_caffe"
    ...
    local lane_detection_path="/smartsim/modules/perception/production/data/perception/camera/models/lane_detector/darkSCNN_caffe"
    ...
    # SMOKE
    if [[ "${TARGET_ARCH}" == "x86_64" ]]; then
        local smoke_volume="smartsim_smoke_volume_${USER}"
        local smoke_image="${DOCKER_REPO}:smoke_volume-yolo_obstacle_detection_model-${TARGET_ARCH}-latest"
        local smoke_path="/smartsim/modules/perception/production/data/perception/camera/models/yolo_obstacle_detector/smoke_libtorch_model"
