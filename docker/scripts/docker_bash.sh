...
function postrun_start_user() {
    ...
    if [ "${USER}" != "root" ]; then
        docker exec -u root "${container}" \
            bash -c '/smartsim/scripts/docker_start_user.sh'
    fi
}

function stop_all_smartsim_containers() {
    local force="$1"
    local running_containers
    running_containers="$(docker ps -a --format '{{.Names}}')"
    for container in ${running_containers[*]}; do
        if [[ "${container}" =~ smartsim_.*_${USER} ]]; then
...
function check_agreement() {
    local agreement_record="${HOME}/.smartsim_agreement.txt"
    if [[ -e "${agreement_record}" ]]; then
        return 0
    fi
    local agreement_file
    agreement_file="${smartsim_ROOT_DIR}/scripts/AGREEMENT.txt"
...
export -f stop_all_smartsim_containers remove_container_if_exists
