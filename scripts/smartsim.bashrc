SMARTSIM_ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
SMARTSIM_IN_DOCKER=false

# If inside docker container
if [ -f /.dockerenv ]; then
  SMARTSIM_IN_DOCKER=true
  SMARTSIM_ROOT_DIR="/smartsim"
fi

export SMARTSIM_CONFIG_HOME="${SMARTSIM_CONFIG_HOME:=$HOME/.SMARTSIM}"

export SMARTSIM_ROOT_DIR="${SMARTSIM_ROOT_DIR}"
export SMARTSIM_IN_DOCKER="${SMARTSIM_IN_DOCKER}"
export SMARTSIM_CACHE_DIR="${SMARTSIM_ROOT_DIR}/.cache"
export SMARTSIM_SYSROOT_DIR="/opt/smartsim/sysroot"

export TAB="    " # 4 spaces

export GPU_SETUP_COMPLETED

source ${SMARTSIM_ROOT_DIR}/scripts/common.bashrc
...
function run() {
  if [ "${VERBOSE}" = yes ]; then
    ...
    local errfile="${SMARTSIM_ROOT_DIR}/.errors.log"
    echo "${@}" >"${errfile}"
    if ! "${@}" >>"${errfile}" 2>&1; then
      local exitcode=$?
      cat "${errfile}" 1>&2
